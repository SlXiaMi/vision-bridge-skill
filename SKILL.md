...
**策略：**
- **独立任务用并行**：每张图回答同一个问题、互不依赖时加 `--parallel`
- **上下文任务用会话**：需要基于前一轮结果追问、对比、递进分析时用 `--session`
- **组合模式是常态**：大多数复杂任务不是纯独立也不是纯上下文——先 `--parallel` 并行扫定位，锁定目标后 `--session` 开会话深挖。parallel 和 session 是同一任务的前后两步，不是二选一
- 按需聚焦：问什么答什么，够了就停
- 追问不带文件路径，结束必须 `--clear`
