...
| | `--parallel` | `--session` |
|---|---|---|
| **优势** | API 并发调用，N 张图耗时 ≈ 最慢那张 | 缓存图片、维持上下文、追问不重传 |
| **劣势** | 不支持会话、不能追问、无法基于前一轮结果调整 | 逐张串行、N 张图耗时 = 累加 |
| **何时用** | 每张图独立回答同一个问题、互不依赖 | 追问、对比、递进分析、需要前一轮信息 |

**并行和会话不是二选一，是同一任务的前后两步。** 单独用都有短板——纯并行不能深挖，纯会话太慢。

### 组合模式（默认策略）

大多数复杂任务都该分两步走。不确定目标位置时尤其如此：

```
第一步：--parallel 并行扫定位 → 快速覆盖大范围，找到目标
第二步：--session 开会话深挖   → 锁定目标后单点突破，可追问可对比
```

**为什么这么用：** 第一步并行扫范围，N 页耗时才等于最慢那页，不会被串行拖累。找到目标后，第二步用会话可以实现多轮深挖——追问细节、对比相邻页、递进分析，这些并行做不到。两步各取所长，整体效率最高。

**示例（PDF 查图）：**
```bash
# 第一步：并行扫，快速定位
vision-bridge.py doc.pdf --pdf-range 370-380 --ask "找图14-1" --parallel --protocol --output json

# 第二步：开会话，深挖目标页
vision-bridge.py doc.pdf --pdf-page 375 --ask "详细解释图中所有标注" --protocol --output json --session auto
vision-bridge.py --ask "公式14-5的物理含义" --session auto-xxx --output json
vision-bridge.py --session auto-xxx --clear
```
...
| 多张独立图 | `--parallel` |
| 追问/对比/递进 | `--session` |
| 大范围未知 | **组合模式：先 parallel 扫，再 session 深** |

**其他策略：**
- **按需聚焦**：用户问什么就只问什么，不问无关信息。信息够了就停，不追问已清楚的
- 追问不带文件路径（图片已缓存，秒级响应）
- 问答结束必须 `--clear`（除非明确要求保留，24 小时后自动过期）
