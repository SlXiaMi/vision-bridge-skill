...
**策略：**
- **能用并行就用并行**：只要每张图的回答不依赖其他图的结果，就加 `--parallel`。多图同时调 API，比串行快几倍。这是默认首选
- **需要上下文才用会话**：追问、对比、递进分析等依赖前一轮结果的场景，才用 `--session`（缓存图片，追问不重传）。会话不支持并行，所以只在确实需要时才开
- **组合是最优解**：大多数复杂任务分两步——先 `--parallel` 并行扫定位，锁定目标后再 `--session` 开会话深挖。既快又深
- **先广后深**：不确定目标位置时先扫一轮，精确定位后再单点突破
- **按需聚焦**：用户问什么就只问什么，够了就停，不追问已清楚的
- 追问不带文件路径（图片已缓存，秒级响应）
- 问答结束必须 `--clear`（24 小时后自动过期）
