...
- **置信度标记**：回答自带 `[确定]` `[可能]` `[推测]` `[无法判断]`，主 AI 可以判断哪些可信、哪些需要追问
- **自适应追问**：一轮不理想就换问法——聚焦单条、锁定位置、换个角度、简化验证。策略自由，语法不变
- **空间锚定**：`x:0.1-0.5,y:0.3-0.6` 按比例坐标锁定图片区域，追问不再模糊
- **两阶段观察**：比较任务先独立描述每张图，再对照差异，从源头切断编造
